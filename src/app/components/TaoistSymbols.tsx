...
export function TaijiSymbol({ className = "", size = 100 }: { className?: string; size?: number }) {
  return (
    ...
    >
      ...
      <path d="M50 2 A48 48 0 0 0 50 98" fill="none" stroke="currentColor" strokeWidth="1" />
      <path d="M50 2 A48 48 0 0 1 50 98 A24 24 0 0 0 50 50 A24 24 0 0 1 50 2" fill="currentColor" />
      <path d="M50 2 A48 48 0 0 0 50 98 A24 24 0 0 1 50 50 A24 24 0 0 0 50 2" fill="white" />
      <circle cx="50" cy="26" r="7" fill="currentColor" />
      <circle cx="50" cy="74" r="7" fill="white" stroke="currentColor" strokeWidth="0.5" />
    </motion.svg>
  );
}
    ...
    <svg viewBox="0 0 100 100" className={className} aria-hidden="true">
      {elements.map((el, i) => (
        <g key={i}>
          ...
          <text x={el.x} y={el.y + 4} textAnchor="middle" fontSize="10" fill="white" fontWeight="bold">
            {el.name}
          </text>
        </g>
      ))}
...
export function CloudPattern({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 200 60" className={className} preserveAspectRatio="none" aria-hidden="true">
      <path d="M0 30 Q25 10 50 30 T100 30 T150 30 T200 30" fill="none" stroke="currentColor" strokeWidth="1" opacity="0.3" />
      <path d="M0 40 Q25 20 50 40 T100 40 T150 40 T200 40" fill="none" stroke="currentColor" strokeWidth="0.5" opacity="0.2" />
    </svg>
  );
}
